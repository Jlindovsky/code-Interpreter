...
trait Functions
{
    ...
    private function getValue(string $name, arg $arg): Variable
    {
        ...
        if ($arg->type == "var") {
            ...
            if ($var !== null) {
                ...
                // Handle the case where $var is null (optional)
                //TODO: exception
    ...
    private function PUSHFRAME($args): void
    {
        ...
        // Push the temporary frame (TF) onto the frame stack (FS)
        ...
        if ($topElement instanceof Frame) {
            ...
            // Handle the case where the top element is not an instance of Frame
            // For example, throw an exception or handle it based on your application's logic
    ...
    private function POPFRAME($args): void
    {
        ...
        // Just before the instanceof check
        ...
        // Set the temporary frame (TF) to the popped frame
    ...
    private function RETURN($args): void
    {
        ...
        if (is_int($poppedValue)) {
            // Assign the integer value to the property
            $this->i = $poppedValue;
        } else {
            // Handle the case where the popped value is not an integer
            // For example, throw an exception or handle it based on your application's logic
            //TODO: exception
    ...
    private function INT2CHAR($args): void
    {
        ...
        if ($unicodeValue >= 0 && $unicodeValue <= 1114111) {
            // Convert the Unicode value to a character using mb_chr()
            $char = mb_chr($unicodeValue, 'UTF-8');
            $symb = new Variable($name, $char, "string");
            $this->$frame->assignValue($symb);
        } else {
            // Invalid Unicode value, raise error
    ...
    private function SETCHAR($args): void
    {
        ...
        // Assuming $sym1->val is a valid integer
        $val = intval($sym1->val);
        ...
        // Concatenate the parts with the new character in between
    ...
    private function DPRINT($args): void
    {
        if ($args[0]->type == "var") {
            ...
            if ($var !== null) {
                ...
                // Handle the case where $var is null (optional)
                // For now, assume the default value is 0
                //TODO: exception
                throw new interpretException("wrong exit code", 57);
            }
        } else {
